...
> [!IMPORTANT]
> **このセットアップガイドは、新規プロジェクト専用です。**
> 既存プロジェクトへの導入手順は提供していません。既存プロジェクトへ `starter/` を直接コピーしないでください。
...
プロダクト用の空フォルダ、または `.git` だけが入った空リポジトリを用意します。
このワークフロー用リポジトリの中でプロダクトを実装するのではなく、プロダクト側へ `starter/` を展開します。

導入スクリプトは、このリポジトリのルートで実行します。
導入先に `.git` 以外のファイルやフォルダがある場合は、既存プロジェクトの上書き事故を避けるため停止します。

PowerShell:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\create-new-project.ps1 C:\path\to\new-project
```

Bash:

```bash
bash scripts/create-new-project.sh /path/to/new-project
```

スクリプトは次を行います。

- `starter/` の全ファイルを導入先へ展開する
- 隠しディレクトリの `.github/` と `.ai-workflow/` も漏れなく展開する
- 展開後に初期設定チェッカーを実行する
- 次に読むべきファイルと、通常タスクを始めてはいけない条件を表示する
