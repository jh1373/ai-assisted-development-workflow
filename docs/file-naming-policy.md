...
このドキュメントは、このリポジトリでファイル名とディレクトリ名を英語で統一する理由を説明します。
...
このリポジトリは、AI駆動開発のためのリポジトリです。
AIに対しては、英語のパスを指定した方が安定します。

例:

```text
docs/quality-gates.md を確認してください。
```
...
このリポジトリでは、英語ファイル名の意味を次の場所で補います。
...
| `docs/design-rationale.md` | 設計理由 | なぜこの構成にしているかを説明する |
| `docs/practical-guide.md` | 実践手順 | 1タスクをどう進めるかを説明する |
| `docs/file-naming-policy.md` | ファイル名の方針 | 英語ファイル名を使う理由と対応表を説明する |
| `docs/quality-gates.md` | 品質チェック | 作業前後に確認すべき関門を説明する |
...
| `docs/pull-request-template.md` | PR確認シートの説明 | PR本文に何を書くかを説明する |
...
`fileNamingPolicy.md` のようなcamelCaseは、Markdown文書の命名としてこのリポジトリでは使いません。
...
このリポジトリでは、英語ファイル名を使うこと自体が目的ではありません。
