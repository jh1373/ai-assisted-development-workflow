...
### 0.5. Project Structure Gateを確認する

`INITIALIZATION_READY` の後、現在の環境に合う構造チェッカーを1つ実行します。
...
- `DIRECTORY_MAP_VERIFIED`: Git確認へ進む
- `DIRECTORY_MAP_PROVISIONAL`: 最初のプロジェクト構築タスクに限り進める。通常機能の追加前にVerified化を計画する
- `DIRECTORY_MAP_DRIFT_DETECTED`: `python scripts/project-structure.py diff` で差分を確認し、地図を修復するまで実装へ進まない
...
今回のタスクに対応する「作業場所の案内」がある場合は、全ファイル一覧より先に確認します。案内がない場合は、implementation planに追加候補として記録します。「処理の流れ」へ影響する変更なら、実装前に対象の流れを確認します。
...
`docs/DIRECTORY_MAP.md` がない場合は通常タスクへ進まず、`.ai-workflow/directory-map.json` から再生成してProject Structure Gateを再実行します。
...
詳しくは [docs/ai-workflow/workflow-modes.md](../docs/ai-workflow/workflow-modes.md) を参照します。
...
詳しくは [docs/ai-workflow/task-records.md](../docs/ai-workflow/task-records.md) を参照します。

認証、権限、課金、個人情報、データ移行、本番設定、依存追加、公開前作業を含む場合は、Strict modeとして扱います。
その場合は [docs/ai-workflow/strict-mode.md](../docs/ai-workflow/strict-mode.md) の確認項目を計画に含めます。
...
- project structure checker: DIRECTORY_MAP_VERIFIED / DIRECTORY_MAP_PROVISIONAL
- git status
