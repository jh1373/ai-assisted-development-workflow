...
このドキュメントは、新規プロジェクトへワークフローを導入した後の `docs/tasks/` の使い方を説明します。
`docs/tasks/` は、このワークフロー自体の保守履歴ではなく、導入先プロジェクトの1タスクごとの作業記録を残す場所です。
...
`PROJECT_BRIEF.md`、`ROADMAP.md`、`INITIALIZATION_REVIEW.md` はプロジェクト全体の初期設定記録です。
1タスクごとの `docs/tasks/` には置きません。
初期設定後の最初の実装や検証から、通常のタスク記録を作成します。
