...
8. `docs/PROJECT_STATUS.md` と `docs/DIRECTORY_MAP.md` を初期化する
9. `docs/INITIALIZATION_REVIEW.md` でレビューする
10. ユーザーの明示承認後だけ状態を `ready` にする
11. チェッカーが `INITIALIZATION_READY` を返すことを確認する
...
2. 現在の開発状況を把握する
3. ROADMAPとPROJECT_STATUSから今回のタスクを1つ選ぶ
4. DIRECTORY_MAPで今回読むべき範囲を絞る
5. `docs/tasks/YYYY-MM-DD-HHMM-task-name/` を作る
6. ユーザーとの認識を揃える
7. AIがWorkflow Modeを判定する
8. `implementation-plan.md` を作ってから実装する
9. テスト、ビルド、手動確認を行う
10. `completion-review.md` とdevlogを残す
11. `workflows/session-end.md` で終了判断を行う
