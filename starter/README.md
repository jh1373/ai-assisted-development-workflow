...
このファイルは、導入後は新規プロジェクト側の `README.md` になります。

まだワークフロー用リポジトリ内の `starter/README.md` として読んでいる場合は、ワークフロー用リポジトリのルートで導入スクリプトを実行してください。
導入先は、空フォルダ、または `.git` だけが入った空リポジトリにします。
...
導入後の新規プロジェクト側でこのファイルを読んでいる場合は、展開はすでに終わっています。
通常タスクを始める前に、`workflows/project-initialization.md` に進んでください。

`starter/` は開発運用の導入パッケージです。
導入後は、新規プロジェクトのルートに開発運用ファイルが配置されます。
プロダクト本体は、その新規プロジェクト側で開発します。
